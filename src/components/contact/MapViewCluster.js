...
export default function MapViewCluster({ restaurants, hoveredRestaurant, selectRestaurant }) {
    ...
    const handleClusterClick = ({ currentTarget: {dataset: { clusterid, latitude, longitude }} }) => {
        const expansionZoom =
            Math.min( supercluster.getClusterExpansionZoom(parseInt(clusterid)), maxZoom );
                    ...
                    {clusters.map(cluster => {
                        ...
                        return isCluster ? (
                            ...
                            <Marker
                                ...
                                lng={longitude}
                            >
                                <svg className={`restaurant-marker ${hoveredRestaurant === cluster.properties.restaurantId ? 'restaurant-marker__hover' : ''}`}
                                    style={{position: "absolute", transform: "translate(-50%, -100%)"}}
                                    viewBox="0 0 365 560"
                                    preserveAspectRatio="none"
                                    data-placeid={cluster.properties.restaurantId}
